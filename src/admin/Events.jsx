  ...
  return (
    <div className="space-y-12 animate-fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white p-12 md:p-16 rounded-[60px] shadow-2xl border border-gray-100 relative overflow-hidden group">
         <div className="absolute top-0 right-0 w-96 h-96 bg-scm-red/5 rounded-full -mr-48 -mt-48 group-hover:scale-150 transition-transform duration-700"></div>
         <div className="flex items-center space-x-10 relative z-10">
            <div className="w-24 h-24 bg-scm-red text-white rounded-[40px] flex items-center justify-center shadow-2xl transform -rotate-3 hover:rotate-0 transition-transform duration-500">
               <Calendar size={48} />
            </div>
            <div>
               <h1 className="text-4xl md:text-6xl font-black text-gray-900 mb-3 leading-tight">Ministry <span className="text-scm-red">Events</span></h1>
        ...
        {loading ? (
          ...
          <div key={event.id} className="group bg-white rounded-[50px] shadow-2xl border border-gray-100 overflow-hidden hover:border-scm-red/20 hover:scale-[1.03] transition-all duration-700 relative flex flex-col">
             ...
             <div className="p-10 flex-grow flex flex-col">
                <h3 className="text-2xl font-black text-gray-900 mb-6 group-hover:text-scm-red transition-colors line-clamp-1">{event.title}</h3>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/70 backdrop-blur-md animate-fade-in overflow-y-auto">
          <div className="bg-white rounded-[60px] shadow-2xl max-w-4xl w-full overflow-hidden relative animate-scale-up my-10 border-4 border-white">
            <div className="bg-scm-red p-14 text-center relative overflow-hidden group">
               ...
               <button onClick={() => setShowModal(false)} className="absolute top-10 right-10 p-4 bg-white/10 text-white hover:bg-white hover:text-scm-red rounded-3xl transition-all shadow-2xl">
                 <X size={24} />
               </button>
               <h2 className="text-5xl font-black text-white mb-3 underline decoration-white/20 underline-offset-8">
                  {editingEvent ? 'Edit Event' : 'New Program'}
               </h2>
               <p className="text-white/40 text-xs font-black uppercase tracking-[0.4em]">Event Management System</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-14 lg:p-20 space-y-12">
              ...
              <div className="pt-10 flex flex-col sm:flex-row gap-8">
                <button
                  ...
                  className="flex-grow py-8 bg-scm-red text-white rounded-[35px] font-black text-xl hover:bg-red-700 transition-all shadow-2xl hover:shadow-red-900/40 transform hover:-translate-y-1 active:scale-95 flex items-center justify-center disabled:opacity-70 group"
                >
