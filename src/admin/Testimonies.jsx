    ...
    <div className="space-y-12 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white p-12 md:p-16 rounded-[60px] shadow-2xl border border-gray-100 relative overflow-hidden group">
         <div className="flex items-center space-x-10 relative z-10">
            <div className="w-24 h-24 bg-scm-gold text-white rounded-[40px] flex items-center justify-center shadow-2xl">
               <Quote size={48} />
            </div>
            <div>
               <h1 className="text-4xl md:text-6xl font-black text-gray-900 mb-3 leading-tight">Ministry <span className="text-scm-gold">Testimonies</span></h1>
               <p className="text-gray-400 font-black uppercase tracking-[0.3em] text-xs">Total Shared: {testimonies.length}</p>
            </div>
         </div>
      </div>
      ...
      <div className="bg-white rounded-[60px] shadow-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="grid grid-cols-1 gap-8 p-12">
          {loading ? (
            ...
            <div key={t.id} className="bg-gray-50 p-10 rounded-[40px] border border-gray-100 hover:bg-white hover:shadow-2xl transition-all duration-500">
               <div className="flex flex-col md:flex-row justify-between gap-8 mb-8">
                  <div className="flex items-center space-x-6">
                     <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-scm-gold shadow-sm border border-gray-100">
                        <User size={28} />
                     </div>
                     <div>
                        ...
                        <div className="flex items-center text-gray-400 font-bold text-sm mt-1">
                           <MessageSquare size={14} className="mr-2 text-scm-blue" /> {t.title}
                        </div>
                     </div>
                  </div>
                  <div className="flex items-center space-x-4">
                     {t.is_approved ? (
                       <button 
                         ...
                         className="px-6 py-3 bg-red-50 text-red-600 rounded-2xl font-black text-xs uppercase tracking-widest border border-red-100 hover:bg-red-600 hover:text-white transition-all"
                       >
                         Unapprove
                       </button>
                     ) : (
                       <button 
                         ...
                         className="px-6 py-3 bg-green-50 text-green-600 rounded-2xl font-black text-xs uppercase tracking-widest border border-green-100 hover:bg-green-600 hover:text-white transition-all"
                       >
                         Approve
                       </button>
                     )}
                     <button onClick={() => handleDelete(t.id)} className="p-4 bg-white text-gray-400 hover:text-scm-red hover:bg-red-50 rounded-2xl border border-gray-100 shadow-xl transition-all">
                        <Trash2 size={24} />
                     </button>
                  </div>
               </div>
               <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-inner">
                  <p className="text-gray-600 text-lg leading-relaxed font-medium italic">"{t.content}"</p>
               </div>
