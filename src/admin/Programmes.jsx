    ...
    <div className="space-y-12 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white p-10 md:p-14 rounded-[50px] shadow-2xl border border-gray-100">
        <div className="flex items-center space-x-8">
           ...
           <div>
              <h1 className="text-4xl md:text-5xl font-black text-gray-900 mb-2">Church <span className="text-scm-blue">Programmes</span></h1>
              <p className="text-gray-400 font-bold uppercase tracking-[0.2em] text-xs">Recurring Weekly & Monthly Activities</p>
           </div>
        </div>
        <button
          onClick={() => openModal()}
          className="px-10 py-5 bg-scm-blue text-white rounded-2xl font-black hover:bg-blue-900 transition-all shadow-xl hover:shadow-scm-blue/40 transform hover:-translate-y-1 active:scale-95 flex items-center justify-center group"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20">
        {loading ? (
          ...
          <div key={prog.id} className="bg-white p-10 rounded-[40px] shadow-2xl border border-gray-100 hover:border-scm-blue/20 hover:scale-[1.03] transition-all duration-500 relative overflow-hidden group">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in">
          <div className="bg-white rounded-[50px] shadow-2xl max-w-2xl w-full overflow-hidden relative animate-scale-up border-4 border-white">
            <div className="bg-scm-blue p-12 text-center relative overflow-hidden group">
               <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-700"></div>
               <button
                 onClick={() => setShowModal(false)}
                 className="absolute top-8 right-8 p-3 bg-white/10 text-white hover:bg-white hover:text-scm-blue rounded-2xl transition-all shadow-xl"
               >
                 <X size={20} />
               </button>
               <h2 className="text-4xl font-black text-white mb-2 underline decoration-scm-red/40 underline-offset-8">
                  {editingProg ? 'Update Programme' : 'New Programme'}
               </h2>
               <p className="text-white/40 text-xs font-black uppercase tracking-[0.3em]">Church Schedule Registry</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-12 lg:p-16 space-y-10">
              ...
              <div className="pt-8 flex flex-col sm:flex-row gap-6">
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-grow py-6 bg-scm-blue text-white rounded-[30px] font-black text-lg hover:bg-blue-900 transition-all shadow-2xl hover:shadow-scm-blue/40 transform hover:-translate-y-1 active:scale-95 flex items-center justify-center disabled:opacity-70 group"
                >
                  {submitting ? (
                ...
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="px-10 py-6 bg-gray-50 text-gray-400 rounded-[30px] font-black hover:bg-gray-100 hover:text-gray-600 transition-all shadow-inner transform active:scale-95"
                >
                  Discard
                </button>
              </div>
